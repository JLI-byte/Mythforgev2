...
import { buildLabelMap, escapeRenpyText, parseDialogueLine, buildAliasMap, buildRenpyScript, toFlagName } from './renpyExport';
...
`);
    ...
    it('leaves the project name raw in the header, which is a comment', () => {
        // A # comment runs to end of line, so quotes and brackets are inert
        // there. Escaping would only make the header harder to read.
        expect(buildRenpyScript([], [], 'A "Quoted" Name'))
            .toContain('# Generated by LoreCanvas — A "Quoted" Name');
    });

    it('reaches the same alias when a speaker is typed in different cases', () => {
        const script = buildRenpyScript([{
            id: 'a', title: 'A', order: 0,
            content: ['Sylvie: Hey.', 'SYLVIE: Again.'].join('\n'),
        }], ['Sylvie'], 'X');
        expect(script).not.toContain('undefined');
        expect(script).toContain('    s "Hey."');
        expect(script).toContain('    s "Again."');
    });
    ...
    it('emits flag names as identifiers, whatever the writer typed', () => {
        const script = buildRenpyScript([{
            ...
            choices: [
                { id: 'c1', text: 'Go', targetSceneId: 'a', setsFlag: 'met bob' },
                { id: 'c2', text: 'Recall', targetSceneId: 'a', requiresFlag: 'met bob' },
            ],
        }], [], 'X');
...
describe('toFlagName', () => {
    it('turns a typed phrase into an identifier', () => {
        expect(toFlagName('met bob')).toBe('met_bob');
    });

    it('never leaves a space, which would be a syntax error', () => {
        expect(toFlagName('told the truth at last')).not.toContain(' ');
    });

    it('prefixes a flag that starts with a digit', () => {
        expect(toFlagName('3rd visit')).toBe('s_3rd_visit');
    });

    it('suffixes a flag that collides with a Ren\'Py keyword', () => {
        expect(toFlagName('return')).toBe('return_flag');
    });

    it('falls back when nothing usable is left', () => {
        expect(toFlagName('???')).toBe('flag');
    });
});
