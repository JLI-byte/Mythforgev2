...
/**
 * Flag id → the identifier it is emitted as, deduped.
 *
 * Slugifying is lossy: "met bob" and "met-bob" both reduce to `met_bob`. Two
 * flags sharing an identifier is not a syntax error — `default` simply does
 * not rebind — so the second flag would silently read and write the first
 * one's state, with nothing to point at. Labels have always deduped; flags
 * must too.
 *
...
function buildFlagNameMap(flags: VNFlag[]): Map<string, string> {
    ...
    const stable = [...flags].sort(
        (a, b) => a.name.localeCompare(b.name) || a.id.localeCompare(b.id),
    );

    for (const flag of stable) {
        const base = toFlagName(flag.name);
    ...
    ordered.forEach((scene, index) => {
        ...
        if (choices.length) {
            ...
            for (const choice of choices) {
                // A condition on a deleted flag drops the guard, so the choice
                // is always offered. Deliberately asymmetric with effects
                // below, which are skipped: hiding the choice instead would
                // strand whatever branch it leads to, and an unreachable
